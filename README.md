...
從導覽列開啟「歷年動畫」（`#/archive`），預設上一個已結束季度。可自由輸入年份（API 年份清單提供輸入建議），選擇冬、春、夏、秋，並依熱門度、評分或原文名稱排序及篩選播出形式／題材。切換條件回到第一頁；收藏卡片顯示既有狀態與進度。

沿用 Jikan / Bangumi Provider、快取、詳細頁及收藏流程。排序先讀完整季度再分頁，首次載入可能較久。熱門度按收藏人數，評分按來源分數；Bangumi 備援依首播日期與標籤查詢，分類可能不同。API 不可用時顯示快取或重試提示，個人紀錄不受影響。

## 巴哈姆特動畫瘋同步

同步達標觀看時，未配對作品會依動畫瘋作品 ID 自動新增至「我的動畫」，已配對作品更新原收藏。觀看平台設定為巴哈姆特動畫瘋。未配對收藏先保存原始名稱、進度與時間，其他公開資訊顯示未定，不依名稱猜測季度。首次歷史匯入可選取多筆，再確認「批次加入我的動畫」；不明集數／未達門檻仍須個別確認。

搜尋、本季、未來及歷年動畫提供「選取本頁未收藏」與批次加入想看／開始觀看。我的動畫可選取篩選結果並批次移除，確認前顯示數量；移除會一併移除觀看紀錄，建議先備份。已移除的巴哈作品不會被下一筆同步立刻重新加入，會回到待確認清單。
