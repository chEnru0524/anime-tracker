...
## 2026-09-17 台灣平台與帳號雲端

- Build 通過；原有搜尋、收藏、批次編輯、歷年新番、備份與動畫瘋測試回歸通過。
- 平台測試：MAL ID 配對、僅 TW 地區、作品地區覆寫、危險 URL 拒絕、同名歧義與不同年份拒絕。
...
## 2026-09-17 批次觀看狀態與舊番整理

- Build 與 28 項單元測試通過；本機 12 項瀏覽器測試通過，原生擴充功能沿用 Linux CI。
- 驗證跨年／跨季、排除本季及未來、非今年／非本季、日期未定、篩選後清除勾選、取消／確認批次狀態及重新整理保存。
- 已看完填滿已知集數、未知總數保留進度，重複完成不重設完成日期；其他狀態保留進度、清除完成日期。評分、備註與最近觀看時間保持不變。
- 360／768／1440px 無橫向溢出，新增流程無未捕捉 JavaScript error。
